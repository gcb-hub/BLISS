...
# validate-6k-coloc.R
#
# Report two numbers for coloc:
#   Number 1 (denominator): rows in data/universe-6k.tsv that coloc can
#                            reasonably answer.
#   Number 2 (numerator):   rows in the denominator validated by the
#                            significant portion of coloc results.
#
# Pipeline:
#   1. Load test/denom-6k-coloc.tsv (from create-denom-6k-coloc.R).
#   2. Load coloc.RData, apply the same QC filter used to build the
#      denominator (drop exception in NO_OVERLAP / PQTL_EMPTY / PQTL_NO_SIGNAL)
#      and translate protein -> gene via lookup-Olink.
#   3. Shrink dat to (gene, TRAIT) pairs in the denominator. Hit iff
#      H4 >= 0.8 (coloc reports a posterior probability, no FDR).
#   4. Mark each denominator row as validated iff any of its candidate
#      (gene, TRAIT) pairs is a hit.
#
# Outputs:
#   stdout: N1, N2, proportion
#   test/validated-6k-coloc.tsv -- denominator file plus a `validated` flag
#
# Run from the project root:
#   Rscript code/validate-6k-coloc.R
# =============================================================================
...
# Load inputs
# -----------------------------------------------------------------------------
denom <- fread("test/denom-6k-coloc.tsv", sep = "\t", quote = "")
...
# -----------------------------------------------------------------------------
# Shrink dat to (gene, TRAIT) in the denominator; significance by H4.
# -----------------------------------------------------------------------------
...
out <- copy(denom)
out[, denom_row := NULL]
fwrite(out, "test/validated-6k-coloc.tsv", sep = "\t", quote = FALSE)

cat("--- coloc validation ---\n")
